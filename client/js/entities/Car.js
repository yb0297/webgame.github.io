class Car extends Phaser.Physics.Arcade.Sprite {
    ...
    createWheels() {
        ...
        wheelPositions.forEach((pos, index) => {
            if (this.scene.textures.exists('wheel')) {
                const wheel = this.scene.add.sprite(this.x + pos.x, this.y + pos.y, 'wheel');
                wheel.setScale(0.8);
                this.wheels.push(wheel);
            } else {
                console.warn('Wheel texture not available, wheels not created');
            }
        });
    }
    ...
    createExhaustEffect() {
        ...
        if (this.scene.textures.exists('coin')) {
            ...
            // Create exhaust effect later when textures are available
            this.exhaustParticles = null;
            console.warn('Coin texture not available, exhaust particles disabled');
        }
    }
